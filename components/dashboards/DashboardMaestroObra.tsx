...
import { Building2, TrendingUp, Hammer, AlertTriangle, Users } from 'lucide-react'
...
export default async function DashboardMaestroObra({ perfil }: Props) {
  const supabase = await createClient()

  // ── 1. Obras donde este maestro es supervisor ────────────────────────────
  ...
  // ── 2. Datos relacionados + asistencias de hoy, en paralelo ─────────────
  const hoy = new Date().toISOString().slice(0, 10) // YYYY-MM-DD

  const [fasesRes, avancesRes, incidenciasRes, reportesRes, asistenciasRes] = await Promise.all([
    obraIds.length > 0
      ? supabase
          .from('fases_obra')
    ...
    obraIds.length > 0
      ? supabase
          .from('asistencias')
          .select('id, obra_id, perfil_id, fecha, hora_entrada, hora_salida, obras(nombre)')
          .in('obra_id', obraIds)
          .eq('fecha', hoy)
  ...
  // ── 2b. Nombres de trabajadores: segunda consulta directa a profiles ─────
  // (evita depender del nombre exacto de la FK constraint en el join anidado)
  const profilesMap = new Map<string, { nombre: string | null; apellidos: string | null }>()
  const perfilIds = [...new Set(asistencias.map((a: any) => a.perfil_id).filter(Boolean))]
  if (perfilIds.length > 0) {
    ...
    for (const p of profilesData ?? []) {
      profilesMap.set(p.id, { nombre: p.nombre, apellidos: p.apellidos })
    }
  }
  ...
  const totalEntradas  = asistencias.filter((a: any) => a.hora_entrada).length
  const totalEnObra    = asistencias.filter((a: any) => a.hora_entrada && !a.hora_salida).length
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        <p className="text-sm" style={ts}>Maestro de Obra · GRUPO LOZCAM S.A.C</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="rounded-2xl p-5" style={cardStyle}>
          ...
          <p className="text-xs" style={ts}>entre tus obras</p>
        </div>
      ...
      <div className="rounded-2xl overflow-hidden mb-4" style={cardStyle}>
        ...
        {obrasList.length === 0 ? (
          <div className="px-5 py-10 text-center">
            <p className="text-sm mb-1" style={ts}>No tienes obras asignadas todavía</p>
      ...
      <div className="rounded-2xl overflow-hidden mb-4" style={cardStyle}>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold" style={tp}>Asistencia de hoy</h2>
          </div>
          {asistencias.length > 0 && (
            <div className="flex items-center gap-3">
              <span className="text-xs" style={ts}>
                <span className="font-semibold" style={tp}>{totalEntradas}</span> entraron
              </span>
              {totalEnObra > 0 && (
                <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-green-500/15 text-green-700 dark:text-green-400">
                  {totalEnObra} en obra
                </span>
              )}
            </div>
        ...
        {asistencias.length === 0 ? (
          <div className="px-5 py-10 text-center">
            <p className="text-sm mb-1" style={ts}>Nadie ha marcado asistencia hoy</p>
            <p className="text-xs" style={ts}>
              {obrasList.length === 0
                ? 'Asigna obras a tu cargo para ver asistencias'
                : 'Las marcaciones aparecerán aquí al registrarse'}
            </p>
          </div>
        ) : (
          <div>
            {asistencias.map((a: any) => {
              const pData = profilesMap.get(a.perfil_id)
            const nombreTrabajador = [pData?.nombre, pData?.apellidos]
                .filter(Boolean)
                .join(' ') || 'Sin nombre'
              const nombreObra  = a.obras?.nombre ?? '—'
              const enObra      = a.hora_entrada && !a.hora_salida
              ...
              return (
                ...
                >
                  {/* Trabajador + obra */}
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium" style={tp}>{nombreTrabajador}</p>
                    <p className="text-xs truncate mt-0.5" style={ts}>{nombreObra}</p>
                  </div>

                  {/* Entrada / Salida */}
                  <div className="flex items-center gap-3 shrink-0 text-right">
                    <div>
                      <p className="text-[10px] uppercase tracking-wide mb-0.5" style={ts}>Entrada</p>
                      <p className="text-xs font-semibold" style={tp}>
                        {formatHora(a.hora_entrada)}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase tracking-wide mb-0.5" style={ts}>Salida</p>
                      {enObra ? (
                        <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-green-500/15 text-green-700 dark:text-green-400">
                          En obra
                        </span>
